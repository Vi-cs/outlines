...
class Regex(Continuation):
    ...
    def create_proposal(
        ...
        if Params.verbose:
            print(f'len(logits):{len(logits[0])} - logits:{logits}')
            print(f'len(mask):{len(mask[0])} - mask:{mask}')
        ...
        if top_1_without_mask_values != top_1_with_mask_values:
            ...
            if Params.verbose:
                ...
                print(f'Without mask : {tokenizer.decode(top_1_without_mask_indices[0])}\nWith mask : {tokenizer.decode(top_1_with_mask_indices[0])}')
